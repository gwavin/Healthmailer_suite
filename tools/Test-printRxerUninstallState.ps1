...
Test-State "No running printRxer process" ($null -eq $processes) "ProcessCount=$(if ($processes) { $processes.Count } else { 0 })"
...
Write-Host "printRxer uninstall state checks passed."
